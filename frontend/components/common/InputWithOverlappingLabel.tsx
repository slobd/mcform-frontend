...
const InputWithOverlappingLabel: NextPage<InputWithOverlappingLabelProps> = ({ wrapperClassName, type, label, name, placeholder, value, onChange, disabled = false, suffix }) => {
    ...
    return (
        <div className={`relative ${wrapperClassName}`}>
            <label
                htmlFor={name}
                className={`absolute left-2 inline-block bg-white px-1 font-normal text-gray-500 transition-all
            ...
            <input
                ...
                name={name}
                id={name}
                className={`block w-full !bg-white rounded-sm border-0 py-2 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-1 focus:ring-inset focus:ring-gray-600 sm:text-sm sm:leading-6 
